...
module.exports = function logRequest(req, res, next) {
  ...
  if (_.isNumber(req._startTime) || _.isDate(req._startTime)) {
    report.diagnostic.middlewareLatency = receivedAt - req._startTime.valueOf();
  }//>-
  ...
  res.once('finish', function onceFinish() {
    ...
    // Since res.getHeaders() is not supported by all versions of the virtual request interpreter
    // in Sails (i.e. for sockets), we fall back to _headers.
    // (note that res._headers is not a documented thing and should not be relied upon!!!!)
